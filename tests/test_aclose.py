...
from tests.tools.func import identity, nothing, throw_func
...
@pytest.mark.asyncio
async def test_aclose_concurrent_operations(
    n_yields: int,
    concurrency: int,
...
async def test_aclose_observe_every() -> None:
    async def pull_1_and_close() -> None:
        async with aclosing(
            stream(range(10))
            .do(asyncio.sleep)
            .observe(every=timedelta(seconds=1))
            .observe(every=timedelta(seconds=1))
            .do(nothing)
            .__aiter__()
        ) as it:
...
@pytest.mark.asyncio
async def test_aclose_on_early_stop(
