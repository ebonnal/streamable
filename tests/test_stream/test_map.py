...
@pytest.mark.parametrize("itype", ITERABLE_TYPES)
def test_map_process_concurrency_ordering(
    as_completed: bool,
    order: Callable[[Iterable], Iterable],
    itype: IterableType,
) -> None:
    with ProcessPoolExecutor(3) as processes:
        sleeps = [0.01, 1, 0.5]
        s = stream(sleeps).map(
            identity_sleep, concurrency=processes, as_completed=as_completed
        )
...
def test_map_process_concurrency_partial_iteration(
    ...
    assert time.perf_counter() - start == pytest.approx(
        sleeps[0] + sleeps[concurrency], rel=0.1
    )
...
@pytest.mark.parametrize("identity_sleep", [identity_sleep, async_identity_sleep][1:])
