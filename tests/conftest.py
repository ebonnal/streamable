...
def complex_stream() -> stream:
    return (
        ...
        .do(async_identity)
        .map(CustomCallable())
        .map(async_identity, as_completed=True)
        .group(100)
        .group(100, by=async_identity)
        .map(star(lambda key, group: group))
        .observe("groups", do=print)
...
def complex_stream_str() -> str:
    ...
    .skip(until=<lambda>)
    .skip(until=async_identity)
    .map(<lambda>, concurrency=1, as_completed=False)
    .map(<lambda>, concurrency=2, as_completed=False)
    .filter(star(bool))
    .filter(star(async_identity))
    .do(<lambda>, concurrency=1, as_completed=False)
    .do(<lambda>, concurrency=2, as_completed=False)
    .do(async_identity, concurrency=1, as_completed=False)
    .map(CustomCallable(), concurrency=1, as_completed=False)
    .map(async_identity, concurrency=1, as_completed=True)
    .group(up_to=100, every=None, by=None)
    .group(up_to=100, every=None, by=async_identity)
    .map(star(<lambda>), concurrency=1, as_completed=False)
    .observe('groups', every=None, do=print)
    .flatten(concurrency=4)
    .map(async_iter, concurrency=1, as_completed=False)
    .flatten(concurrency=4)
    .map(<lambda>, concurrency=1, as_completed=False)
    .throttle(64, per=datetime.timedelta(seconds=1))
    .observe('foos', every=10)
