...
for times, s in (
    ...
    (1, stream(range(100) for _ in range(N // 100)).flatten(concurrency=2)),
):
    duration = timeit.timeit(lambda: consume(s), number=times) / times
    print(str(s)[:100])
    print(f"is {duration / baseline:.3f}x slower than builtins.map")
