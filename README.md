...
> [!IMPORTANT]
> The `Stream`'s interface is minimalist: Features such as the creation of the stream's source or destination are not the responsability of `streamable`, it is meant to be combined with libraries like `csv`, `json`, `pyarrow`, `requests`, ...

> [!NOTE]
> **`async` twin operations:** Each operation that takes a function (e.g. `.map`) also has a version that accepts an `async` function (e.g. `.amap`). You can mix both types of operations on the same `Stream`, and then consume it as either an `Iterable` or an `AsyncIterable`.
...
> [!NOTE]
> **Ordering**: it yields results in the upstream order (FIFO), set the parameter `ordered=False` to yield results as they become available (*First Done, First Out*).
