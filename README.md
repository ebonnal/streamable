...
A simple `stream(range(...))` yields ~150M elems per second, here are rough estimates of the throughput of operations applied on this base stream:
...
**100k-1M elems/s:**
- `.map`/`.do`/`.flatten` with `concurrency`
- `.group` with `within`
- `.buffer`
