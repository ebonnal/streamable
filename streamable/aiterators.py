...
class CatchAsyncIterator(AsyncIterator[T]):
    def __init__(
        self,
...
class DistinctAsyncIterator(AsyncIterator[T]):
    def __init__(
        self, iterator: AsyncIterator[T], key: Optional[Callable[[T], Any]]
    ) -> None:
        validate_aiterator(iterator)
        self.iterator = iterator
        self.key = wrap_error(key, StopAsyncIteration) if key else None
...
class ConsecutiveDistinctAsyncIterator(AsyncIterator[T]):
    def __init__(
        self, iterator: AsyncIterator[T], key: Optional[Callable[[T], Any]]
    ) -> None:
        validate_aiterator(iterator)
    ...
    async def __anext__(self) -> U:
        while True:
            try:
                ...
                self._current_iterator_elem = iter_wo_stopiteration(await anext(self.iterator))
...
class GroupbyAsyncIterator(_GroupAsyncIteratorMixin[T], AsyncIterator[Tuple[U, List[T]]]):
    def __init__(
        self,
        iterator: AsyncIterator[T],
        key: Callable[[T], U],
        ...
        super().__init__(iterator, size, interval)
        self.key = wrap_error(key, StopAsyncIteration)
...
class PredicateSkipAsyncIterator(AsyncIterator[T]):
    def __init__(self, iterator: AsyncIterator[T], until: Callable[[T], Any]) -> None:
        validate_aiterator(iterator)
        self.iterator = iterator
        self.until = wrap_error(until, StopAsyncIteration)
        self._done_skipping = False
...
class CountAndPredicateSkipAsyncIterator(AsyncIterator[T]):
    def __init__(
        self, iterator: AsyncIterator[T], count: int, until: Callable[[T], Any]
    ) -> None:
        validate_aiterator(iterator)
        validate_count(count)
        self.iterator = iterator
...
class _ConcurrentMapAsyncIterable(
    Generic[T, U], ABC, AsyncIterable[Union[U, _RaisingAsyncIterator.ExceptionContainer]]
):
    """
    ...
    async def __aiter__(self) -> AsyncIterator[Union[U, _RaisingAsyncIterator.ExceptionContainer]]:
        with self._context_manager():
            ...
            with suppress(StopAsyncIteration):
                while len(future_results) < self.buffersize:
                    future_results.add_future(self._launch_task(await anext(self.iterator)))
...
class _OSConcurrentMapAsyncIterable(_ConcurrentMapAsyncIterable[T, U]):
    def __init__(
        self,
        iterator: AsyncIterator[T],
...
class OSConcurrentMapAsyncIterator(_RaisingAsyncIterator[U]):
    def __init__(
        self,
        iterator: AsyncIterator[T],
        transformation: Callable[[T], U],
...
class _AsyncConcurrentMapAsyncIterable(_ConcurrentMapAsyncIterable[T, U]):
    def __init__(
        self,
        iterator: AsyncIterator[T],
...
class _ConcurrentFlattenAsyncIterable(
    ...
    async def __aiter__(self) -> AsyncIterator[Union[T, _RaisingAsyncIterator.ExceptionContainer]]:
