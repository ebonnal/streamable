...
class _GroupByEveryAsyncIterable(
    ...
    def _timeout(self) -> Optional[float]:
        if self._groups:
            oldest_group_creation_time = next(iter(self._groups.values()))[0]
            return (
                oldest_group_creation_time + self._every_seconds - time.perf_counter()
            )
        return None
            ...
            while True:
                try:
                    try:
                        ...
                        if timeout is None:
                            elem = await self._lazy_next_elem.get()
                        else:
                            elem = await asyncio.wait_for(
                                self._lazy_next_elem.get(), timeout=timeout
                            )
...
class GroupByEveryAsyncIterator(RaisingAsyncIterator[Tuple[U, List[T]]]):
    def __init__(
        self,
        iterator: AsyncIterator[T],
        up_to: Optional[int],
        ...
        super().__init__(
            _GroupByEveryAsyncIterable(iterator, up_to, by, every).__aiter__()
        )
