from typing import Iterable, Iterator, TypeVar, cast, AsyncIterator
...
class AsyncIteratorVisitor(Visitor[AsyncIterator[T]]):
    ...
    def visit_stream(self, stream: Stream[T]) -> AsyncIterator[T]:
        aiterable = stream._source() if callable(stream._source) else stream._source
        _util.validate_aiterable(aiterable)
